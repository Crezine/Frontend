...
const TicketCheckoutView: React.FC<ViewProps> = ({ navigate: parentNavigate }) => {
  ...
  /**
   * REFERENCE: Function to handle sending email via backend API
   */
  const handleSendEmail = async () => {
    ...
    // MOCK API CALL
    console.log(`Sending ticket for ${ticketData.eventName} to ${email}...`);
    ...
    try {
      ...
      if (response.ok) alert("Ticket sent to your email!");
    } catch (err) {
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center font-montserrat p-4 md:p-8">
          ...
          {showTicketModal && (
            <div className="fixed inset-0 z-[130] flex items-center justify-center p-4">
              ...
              <motion.div 
                ...
                className="relative z-[140] w-full max-w-[620px] max-h-[90vh] overflow-visible"
              >
                <button 
                  onClick={() => setShowTicketModal(false)}
                  className="absolute -top-3 -right-3 z-[150] p-2.5 bg-white border border-black rounded-full text-black hover:bg-black hover:text-white transition-all shadow-xl"
                >
                  <FiX size={18} />
                </button>
                <div className="w-full h-full overflow-y-auto no-scrollbar rounded-[2rem]">
        ...
        >
          ...
          <p className="text-sm font-normal text-secondary mb-10 font-montserrat px-4">
            Enter your email to receive your PDF ticket
          </p>
          ...
          <div className="w-full max-w-sm space-y-4">
            <div className="flex flex-col items-start gap-2 w-full text-left">
              ...
              <input 
                ...
                className="w-full h-12 px-4 bg-transparent border border-black rounded-xl text-xs font-normal focus:outline-none focus:border-secondary transition-colors text-black"
                placeholder="Enter your email"
              />
            </div>

            {/* Send Ticket via Email Button */}
            <button 
              onClick={handleSendEmail}
              disabled={isSending}
              className="w-full py-3 bg-secondary text-white rounded-full text-sm font-normal tracking-widest hover:opacity-90 transition-all shadow-md font-montserrat disabled:opacity-50"
            >
              {isSending ? "Sending..." : "Send ticket to email"}
            </button>

            {/* Download Ticket PDF Button */}
            <PDFDownloadLink 
              document={<TicketPDF data={ticketData} />} 
              fileName={`${ticketData.orderId}-ticket.pdf`}
              className="w-full py-3 bg-white border border-black text-black rounded-full text-sm font-normal tracking-widest hover:bg-black hover:text-white transition-all font-montserrat text-center block"
            >
              {({ loading }) => (loading ? 'Preparing PDF...' : 'Download PDF ticket')}
            </PDFDownloadLink>

            <div className="flex justify-center gap-6 pt-2">
              <button 
                onClick={() => setShowTicketModal(true)}
                className="text-xs font-normal text-black hover:text-secondary underline decoration-secondary/30 transition-all font-montserrat tracking-widest uppercase"
              >
                Preview ticket
              </button>
              <button 
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center font-montserrat p-4 md:p-8">
      ...
      <div className="relative w-full max-w-xl flex flex-col items-center">
        ...
        >
          <button onClick={() => navigate(-1)} className="absolute top-4 right-4 z-[120] p-2 bg-white border border-black rounded-full text-black hover:bg-black hover:text-white transition-all shadow-sm">
            <FiX size={16} />
          </button>
                    ...
                    <>
                      <div className="space-y-4">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <input type="email" placeholder="Enter your email" className="w-full h-[44px] px-4 bg-transparent border border-black/40 rounded-xl text-xs font-normal focus:outline-none focus:border-secondary transition-colors text-black" />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          <label className="text-[10px] tracking-wide text-black font-normal ml-1">Card number</label>
                          <input type="text" placeholder="0000 0000 0000 0000" className="w-full h-[44px] px-4 bg-transparent border border-black/40 rounded-xl text-xs font-normal focus:outline-none focus:border-secondary transition-colors text-black" />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <div className="flex flex-col gap-1.5"><label className="text-[10px] tracking-wide text-black font-normal ml-1">Expiration</label><input type="text" placeholder="MM / YY" className="w-full h-[44px] px-4 bg-transparent border border-black/40 rounded-xl text-xs font-normal focus:outline-none focus:border-secondary transition-colors text-black" /></div>
